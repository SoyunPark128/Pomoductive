...
namespace Pomoductive
{
    public sealed class Section : ContentControl
    {
        ...
        private void UpdateControl()
        {
            if (_content != null)
            {
                GridLength _zero = new GridLength(0);
                GridLength _auto = new GridLength(1, GridUnitType.Auto);
                _content.RowDefinitions[0].Height = Header == null ? _zero  : _auto;
                _content.RowDefinitions[2].Height = Footer == null ? _zero : _auto;
                if (_button != null)
                {
